...
import { IPublishOptions, publish } from "./publish";
...
const npmUpdateClients = async (
  ...
  for (const client of clientProjects) {
    try {
      ...
      const isFileDependency = dependencies.authorization.startsWith("file:");
      ...
      if (isFileDependency) {
        ...
        try {
          const libsDir = path.dirname(client.packageJsonPath) + "/shared/libs";
  ...
  if (failedClients.length) {
    console.log(
      `   ❌ Failed ${failedClients.length} projects: ${failedClients
        .map((f) => f.name)
        .join(", ")}`
    );
  }
